...
<script>
    ...
    for (let i = 1; i <= names.length; i++) {
        ...
        document.getElementById(`weeknumber`).textContent = `${yearNumber}年第${weekNumber}周组会汇报顺序`;     document.getElementById(`weeknumber`).textContent = `${}年第${weekNumber}周组会汇报顺序`;
...
<script>
    ...
    for (let i = 1; i <= names.length; i++) {
        ...
        const randomName = names[i-1];
        const yearNumber = today.getFullYear();
        document.getElementById(`weeknumber`).textContent = `${yearNumber}年第${weekNumber}周组会汇报顺序`;
        document.getElementById(`number${i}`).textContent = `${i}: ${randomName}`;
    }
</script>
